...
import 'package:audio_video_trimmer/src/file_formats.dart';
import 'package:audio_video_trimmer/src/source.dart';
import 'package:audio_video_trimmer/src/storage_dir.dart';
...
class Trimmer {
  ...
  Future<void> loadFile(
      {@required File file, @required SourceType sourceType}) async {
    ...
    if (currentSourceType == SourceType.Audio) {
      if (currentFile != null) {
        ...
        await audioPlayer.setUrl(currentFile.path, isLocal: true);
        await audioPlayer.resume();
        // delayed so that the audio duration is correct
        await Future.delayed(Duration(milliseconds: 20));
        var getTotalDuration = await audioPlayer.getDuration();
        print("gg $getTotalDuration");
        totalDuration = Duration(milliseconds: getTotalDuration);
  ...
  Future<String> saveTrimmedFile({
    ...
    final String _videoPath = currentFile.path;
    ...
    if (outputFormat == null) {
      outputFormat = currentSourceType == SourceType.Video
          ? VideoFileFormat.mp4
          : AudioFileFormat.mp3;
    ...
    if (ffmpegCommand == null) {
      ...
      if (outputFormat == VideoFileFormat.gif) {
        if (fpsGIF == null) {
          fpsGIF = 10;
        }
        if (scaleGIF == null) {
